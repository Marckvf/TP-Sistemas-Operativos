...
void server_online(){
  //setbuf(stdout,NULL);
      ...
      int fd_server;
      char tipo_mensagem;
      MFC_login msg_login;                             //estrutura com 2 arrays, para user e pass.
      cli_online array_cli_online[NMAXPLAY];
      int pos_atual_array_cli_online = -1;      // indica nº jogadorfes ativos. Serve tambem de indice.
      ...
      while(1){
        ...
        if(read(fd_server,&tipo_mensagem,sizeof(tipo_mensagem)) == -1){
          perror("A func read nao consegiui ler. Servidor vai fechar.");
          close(fd_server);
          //avisar clientes;
          //desligar clientes;
          unlink(PIPE_DO_SERVIDOR);
        ...
        if(tipo_mensagem == 'l' ){ //login

            // Guarda em estrutura Temporária.
            if(read(fd_server,&msg_login,sizeof(msg_login)) == -1){
              perror("A func read nao consegiui ler. Servidor vai fechar.");
              close(fd_server);
              //avisar clientes; atraves de sinal?
              //desligar clientes;
              unlink(PIPE_DO_SERVIDOR);
            }

            printf("\n%s, ",msg_login.pipe_name );
            printf("%s, ", msg_login.user);
            printf("%s\n", msg_login.pass);

            // verificar autenticidade
            f_ptr_registo = fopen("registo.txt", "r");
            if(!f_ptr_registo){
              perror("Impossivel abrir registo.txt.");
            }
            match = 0;
            while(fscanf(f_ptr_registo, "%s %s", id_f, pw_f)!=EOF){			//Este ciclo guarda em id_t e pw_t o id e pass de cada linha, até ao fim ficheiro
              if(strcmp(msg_login.user, id_f) == 0 && strcmp(msg_login.pass, pw_f) == 0){				          //Se id e id_t forem iguais
          	     match = 1;
            }
            }fclose(f_ptr_registo);


            // se tivermos aingido o NMAXPLAY
            if (pos_atual_array_cli_online + 1 >= NMAXPLAY){
              perror("ja exitem 20 jogadres online");
              //avisar cliente; atraves de sinal?
              continue;
            }
            ...
            // #########################################
            // Falta garantir que nao uma entrada de login do mesmo processo.
            // Ver estrategia para saber que users estao com login sem estaremn a jogar.
            // #########################################




            //podemos adicionar mais 1 player, 1º Garantir que nao vão haver erros a abir fifo e escrever.
            else{

              if(fdd = open(msg_login.pipe_name, O_WRONLY) == -1){
              //if(array_cli_online[pos_atual_array_cli_online + 1].fd = open(msg_login.pipe_name, O_WRONLY) == -1){
                perror("impossivel abrir o pipe do cliente ('pipe_name') recebido no processamento de login");
                //avisar cliente. tem de ser sinal.
                array_cli_online[pos_atual_array_cli_online + 1].fd = 0; // preciosidade para detetar possiveis erros.
                continue;
              }
              ...
              // ###################             ERRO                ######################
              // está a enviar para o proprio pipe (server) em vez de ser p/ pipe cliente
              // ########################################

              puts("aqui");
              if(fd_server == array_cli_online[pos_atual_array_cli_online+1].fd)
                puts("fiedescriptores iguais!!!!!!!!");
              tipo_mensagem = 'p';
              printf("%d\n",pos_atual_array_cli_online );

              if(write(fdd, &tipo_mensagem, sizeof(tipo_mensagem)) == -1){
              //if(write(array_cli_online[pos_atual_array_cli_online+1].fd, &tipo_mensagem, sizeof(tipo_mensagem)) == -1){
                perror("O login e valido, mas foi impossivel informar o cliente. O login nao sera guardado.");
                array_cli_online[pos_atual_array_cli_online + 1].fd = 0;
                //avisar cliente. com sinal?
                continue;
              }
              puts("\naqui");

              // Se é valido, há espaço, e foi comunicado ao cliente, então adicionar à estrutura clientes online;
              strcpy(array_cli_online[pos_atual_array_cli_online+1].pipe_name, msg_login.pipe_name);
              pos_atual_array_cli_online++;
        ...
        else if ( tipo_mensagem == '\n' ){
          printf("\nTipo de mensagem desconhecido!\n");
          continue;
        }
        else
          printf("\nrecebi outra coisa tambem\n");
      }
